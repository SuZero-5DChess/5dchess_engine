...
constexpr bitboard_t rook_attack_prototype(int xy, bitboard_t blocker)
{
    bitboard_t result = 0;
    int x = xy%BOARD_LENGTH, y = xy/BOARD_LENGTH;
...
constexpr bitboard_t bishop_attack_prototype(int xy, bitboard_t blocker)
{
    bitboard_t result = 0;
    int x = xy%BOARD_LENGTH, y = xy/BOARD_LENGTH;
...
consteval std::array<bitboard_t, rook_data_size> gen_rook_data()
{
    ...
    for(int pos = 0; pos < BOARD_SIZE; pos++)
    {
        ...
        bitboard_t total_subsets = bitboard_t(1) << n; // 2^n subsets
        ...
        for(bitboard_t subset = 0; subset < total_subsets; ++subset)
        {
            ...
            //assert(data[index<<(BOARD_BITS*2) | pos] == 0);
            data[index<<(BOARD_BITS*2) | pos] = rook_attack_prototype(pos, submask);
            //std::cerr << "set " << index << " to:" << bitboard_to_string(data[index<<(BOARD_BITS*2) | pos]) << std::endl;
        }
    }
    return data;
}
...
//    std::cerr << "\n\ncell " << char('a'+pos%8) << char('1'+pos/8) << "\n";
//    std::cerr << bitboard_to_string(blocker) << "index = " << index << std::endl;
//    std::cerr << "expected:\n" << bitboard_to_string(rook_attack_prototype(pos, blocker));
//    std::cerr << "also expected:\n" << bitboard_to_string(rook_attack_prototype(pos, blocker & rook_mask(pos)));
//    std::cerr << "got:\n" << bitboard_to_string(rook_data[index<<(BOARD_BITS*2) | pos]);
