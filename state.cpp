    ...
    std::visit(overloads {
        ...
        [&](std::tuple<vec4, vec4> data)
        {
            ...
            if(d.l() == 0 && d.t() == 0)
            {
                ...
                m.get_timeline(p.l()).push_back(std::make_shared<board>(std::move(b)));
            }
            // non-branching superphysical move
            else if(q.t() == m.timeline_end[multiverse::l_to_u(q.l())])
            {
                ...
                m.get_timeline(q.l()).push_back(std::make_shared<board>(std::move(c)));
            }
            //branching move
            else
            {
